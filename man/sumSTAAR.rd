...
\description{
STAAR procedure developed by Li et al. (2020) adapted to use on summary statistics
}
\usage{
sumSTAAR(score.file, gene.file, genes = "all", cor.path = "cor/",
tests = c("BT", "SKAT", "ACAT"), annotation_phred = paste0("ANNO", 1:10),
beta.par.matrix = rbind(c(1, 1), c(1, 25)), n = NA, write.file = FALSE,
quiet = FALSE)
}
...
\arguments{
	...
	and 'ACAT', same as in original STAAR. ACAT performs with gen.var.weights = "af" for consistency with original STAAR.}

	\item{annotation_phred}{a character vector to define a set of annotations to be used. Annotations should be initially passed to
	\code{prep.score.files} (see \code{prep.score.files()} function) as input file columns "ANNO1", "ANNO2", "ANNO3" etc.
	The same naming should be used for \code{annotation_phred} argument.}
	...
	\item{n}{size of the sample on which summary statistics were obtained. Not required by default. Should be assigned if 'PCA' or 'FLM'
...
\details{
The STAAR procedure has been recently proposed by Li et al. (2020) and described in detail therein. It calculates a set of P values using a range of gene-based tests, beta distribution weights parameters, and weighting by each of 10 functional annotations. The P values are then combined using Cauchy method (see \code{ACAT.pval()} function).
}
\value{
	
	a data frame of size (n.genes x (n.tests x n.beta.pars x (n.annotations + 1) + n.tests + 2)) containing P values
	...
	gene # gene symbol\cr
	BT.1.1.ANNO0 # P value for burden test with beta.par = c(1, 1) and no annotations\cr
	BT.1.1.ANNO1 # P value for burden test with beta.par = c(1, 1) and weighted by first annotation\cr
	BT.1.1.ANNO2 # P value for burden test with beta.par = c(1, 1) and weighted by second annotation\cr
	...\cr
	BT.1.1.ANNO10 # P value for burden test with beta.par = c(1, 1) and weighted by tenth annotation\cr
	BT.1.1.STAAR # combined P value of all burden tests with beta.par = c(1, 1)\cr
	BT.1.25.ANNO0 # P value for burden test with beta.par = c(1, 25) and no annotations\cr
	BT.1.25.ANNO1 # P value for burden test with beta.par = c(1, 25) and weighted by first annotation\cr
	...\cr
	BT.1.25.ANNO10 # P value for burden test with beta.par = c(1, 25) and weighted by tenth annotation\cr
	BT.1.25.STAAR # combined P value of all burden tests with beta.par = c(1, 25)\cr
	SKAT.1.1.ANNO0 # P value for SKAT with beta.par = c(1, 1) and no annotations\cr
	SKAT.1.1.ANNO1 # P value for SKAT with beta.par = c(1, 1) and weighted by first annotation\cr
	...\cr
	SKAT.1.1.ANNO10 # P value for SKAT with beta.par = c(1, 1) and weighted by tenth annotation\cr
	SKAT.1.1.STAAR # combined P value of all SKATs with beta.par = c(1, 1)\cr
	SKAT.1.25.ANNO0 # P value for SKAT with beta.par = c(1, 25) and no annotations\cr
	...\cr
	SKAT.1.25.ANNO10 # P value for SKAT with beta.par = c(1, 25) and weighted by tenth annotation\cr
	SKAT.1.25.STAAR # combined P value of all SKATs with beta.par = c(1, 25)\cr
	ACAT.1.1.ANNO0 # P value for ACAT with beta.par = c(1, 1) and no annotations\cr
	ACAT.1.1.ANNO1 # P value for ACAT with beta.par = c(1, 1) and weighted by first annotation\cr
	...\cr
	ACAT.1.1.ANNO10 # P value for ACAT with beta.par = c(1, 1) and weighted by tenth annotation\cr
	ACAT.1.1.STAAR # combined P value of all ACATs with beta.par = c(1, 1)\cr
	ACAT.1.25.ANNO0 # P value for ACAT with beta.par = c(1, 25) and no annotations\cr
	...\cr
	ACAT.1.25.ANNO10 # P value for ACAT with beta.par = c(1, 25) and weighted by tenth annotation\cr
	ACAT.1.25.STAAR # combined P value of all ACATs with beta.par = c(1, 25)\cr
	sumSTAAR.ACAT_O # combined P value of all 'ANNO0' gene-based tests (without weighting by annotations)\cr
...
score.file <- system.file("testfiles/CFH.scores.anno.vcf.gz",
	package = "sumFREGAT")
gene.file <- system.file("testfiles/NCBI37.3.geneFile.txt.gz",
	package = "sumFREGAT")

res <- sumSTAAR(score.file, gene.file = "hg19", genes = "CFH", cor.path)
