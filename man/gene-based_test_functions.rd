...
burden test ('BT'), principal component analysis-based test ('PCA'),
functional linear model-based test ('FLM'), multiple linear regression ('MLR'), 
Bonferroni correction test ('simpleM'),
...
\arguments{
	...
	\item{gene.file}{name of a text file listing genes in refFlat format. If not set, hg19 file
	...
	If genotypes are not available, correlations can be approximated through reference samples [ref].
	...
	'se.beta' - scores weighted by variance of genotypes estimated from P values and effect sizes (regression coefficients) if provided by user.
	This results in a test analogous to a standard SKAT.
	'af' - scores weighted by variance of genotypes calculated as AF * (1 - AF), where AF is allele frequency. It is a way to approximate
	the standard SKAT test in case when betas are not available.
	Unweighted sum chi-square test gives more weights to common variants compared with SKAT-like tests.}

	\item{method}{the method for computing P value in kernel-based tests. Available methods are "kuonen", "davies" and "hybrid" (see Details).
	Default = "kuonen".}
...
\details{
	...
	A hybrid approach was recently proposed by Wu et al. [2016]. It uses the Davies' method with high accuracy,
